...
test.describe("Hashtags Page", () => {
  ...
  test("should load hashtags page successfully", async ({ page }) => {
    // Check main heading
    await expect(
      page.getByRole("heading", { name: "Trending Hashtags" }),
    ).toBeVisible();

    // Check description
    await expect(
      page.getByText("Explore popular topics and conversations"),
    ).toBeVisible();
  });
  ...
  test("should display top hashtags section", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Top Hashtags" }),
    ).toBeVisible();
  ...
  test("should select a hashtag and display tweets", async ({ page }) => {
    ...
    if (isVisible) {
      ...
      await expect(
        page.getByText(/Tweets with #|No tweets found/),
      ).toBeVisible();
    }
  });

  test("should have active hashtags link in sidebar", async ({ page }) => {
    const hashtagsLink = page
      .locator("aside")
      .getByRole("link", { name: "Hashtags" });
