...
test.describe('Analytics Page', () => {
  ...
  test('should load analytics page successfully', async ({ page }) => {
    ...
    // Check description
    await expect(page.getByText('Deep insights into Twitter network data')).toBeVisible();
  });
  ...
  test('should display network metrics section', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Network Metrics' })).toBeVisible();
  ...
  test('should display top hashtags analysis', async ({ page }) => {
    // Check section heading
    await expect(page.getByRole('heading', { name: 'Top Hashtags Analysis' })).toBeVisible();
  ...
  test('should have active analytics link in sidebar', async ({ page }) => {
    const analyticsLink = page.locator('aside').getByRole('link', { name: 'Analytics' });
